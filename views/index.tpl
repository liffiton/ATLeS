...
<table class="table table-hover" style="max-width: 700px;">
    ...
    <tr>
        <th>
            <button type="button" class="btn btn-xs text-muted" id="selectallbutton" onclick="toggle_select_all();" title='Select All'>
...
%for path, points, img_count in tracks:
    <tr>
        <td>
        %if img_count:
            <button type="button" class="btn btn-xs text-muted selectbutton" data-path='{{path}}' onclick="toggle_select(this);" title='Select'>
                <span class="glyphicon glyphicon-ok"></span>
            </button>
        %end
        </td>
...
<div>
    <p>
        <b>With selection:</b>
        <button type="button" class="btn btn-sm disabled" id="comparebutton" onclick="do_compare();">
            Compare
        </button>
    </p>
    <p>
        <b>Global actions:</b>
        <button type="button" class="btn btn-sm" title="Re-analyze All" onclick="do_post('/analyze_all/', null, 'This will take a long time with no feedback while running.');">
            <span class="glyphicon glyphicon-refresh"></span>
            All
        </button>
    </p>
